...
def drawWalls(surface):
    ...
    for y in range(HEIGHT+1):
        ...
        surface.blit(wallblockdark, ((WIDTH+1) *
                                     snk.BLOCK_SIZE+5, y*snk.BLOCK_SIZE+5))
    ...
    for x in range(WIDTH+2):
        ...
        surface.blit(wallblockdark, (x*snk.BLOCK_SIZE +
                                     5, (HEIGHT+1)*snk.BLOCK_SIZE+5))
...
while True:
    screen.blit(gameovertext, ((WIDTH-4)*snk.BLOCK_SIZE /
                               2, HEIGHT*snk.BLOCK_SIZE/2))
    pg.display.flip()
    event = pg.event.wait()
    if event.type == pg.KEYDOWN:
        if event.key == pg.K_ESCAPE:
